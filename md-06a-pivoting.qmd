...
2.  What's the `tidyverse` Package? Describe in maximum two sentences below. -\> a set of packages to arrange and "clean" data. All the packages work with common data representations and "API" design to avoid any inconsistencies and errors when using multiple packages on the same dataset
...
1.  Starting with the `waste_data_long_mean` object, execute the code and observe the output in the Console. Would you publish this table in this format in a report? -\> no, the names of the columns should be changed to a "publishable" format such as "Category of income" for the first column for example
...
```{r}
waste_data_long_mean |> 
  ggplot(aes(x = mean_percent, y = waste_category, fill = income_cat)) +
